  ...
  "team_hypotheses": [
    {
      "id": "H1",
      "claim": "<团队级判断>",
      "evidence_sources": ["single_facts", "pair_relationship_facts", "business_context"],
      "validation_metric": "<30/60/90天可观察指标>",
      "downgrade_condition": "<什么情况出现就说明该判断要降级>"
    }
  ],
  "validation_plan": {
    "review_window_days": 90,
    "checkpoints": ["30天", "60天", "90天"],
    "required_feedback": ["角色是否落地", "客户入口是否变清楚", "钱账和交付责任是否冻结", "城市试点是否推进"]
  },
  "writing_boundaries": {
...
- 可验证假设：每条核心判断要落成 `team_hypotheses`，写明证据来源、观察指标和降级条件。
- 复盘计划：写明 30/60/90 天要回收什么反馈；没有实际动作或实际反馈时，不把团队判断晋升为全局知识。
...
团队报告交付后，不立刻把判断写进全局知识库。先在外部 run 收集复盘材料：

- 30 天：是否有人真正承担了报告建议的角色；客户入口、交付责任和钱账边界有没有更清楚。
- 60 天：试点是否推进；若没有推进，阻塞来自资源、预算、权责、城市、家庭还是个人意愿。
- 90 天：团队结构是否更稳；是否出现新增成员、退出成员、城市变化或公司层资源变化。
- 每条反馈必须能对应 `team_hypotheses.id`；不能对应的反馈只进入 dialogue note，不直接晋升知识库。
- 复盘候选必须去掉成员姓名、生日、城市、公司、本机路径和报告原文，只保留可复用机制、反例和降级条件。
