...
- 第 01/02 章必须有足够的本 case 事实锚点：四柱/月令/当前大运、紫微命身/四化、西占个人行星/相位、时间敏感项或已知现实问题。若开篇主要依赖“需要被理解、边界感、安全感、不适合被消耗、不太会表达、稳定地爱”等高频话术，而没有具体证据，视为套话风险。
- 每个核心判断有事实依据和现实场景。
...
- 未来几年写趋势和准备，不写绝对预言。
- 如果报告包含时间建议，必须区分大运、流年、流月和流日：年度/月度写行动节奏和准备，流日只用于已确定事项的短窗口；正文必须写清前置条件、避坑提醒和复盘指标，不承诺无条件成功。
- `flow_timing_report` 必须用 `templates/flow-timing-report-template.md` 的结构：一句话总判断、流月差异、重点日期、每日表、使用边界。主交付为 HTML，Markdown 作为源稿。
- `flow_timing_report` 交付前运行 `scripts/validate_flow_timing_report.py` 检查 Markdown 和 HTML；随机抽样日不能只有“注意、适合、谨慎”，至少命中 5 个 case-specific 指纹，不得出现“结构排雷、资源谈判、压力落地、注意沟通、适合推进、谨慎合作、大桃花、大机会、大贵人、大转折”等泛化话术。
- 健康只写作息、压力、精力管理和就医意识。
...
- 如果追问要求“哪天做更好”，必须先确认事项已经确定、候选时间范围、现实准备和不可变限制；否则只给年度/月度节奏，不给流日窗口。
