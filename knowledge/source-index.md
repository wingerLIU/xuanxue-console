...
| `SRC-PROJECT-KNOWLEDGE-CONTEXT` | `scripts/build_knowledge_context.py`; `scripts/create_followup_context.py`; `scripts/create_retrospective_intake.py`; `schemas/knowledge_context.schema.json`; `schemas/retrospective_intake.schema.json`; `knowledge/completeness/retrospective-requirements.json` | curated | 按模块生成本次 case 必读知识文件、来源、blocker、追问上下文、复盘采集计划、机器可读追问库、建议 `target_artifacts` 和外部反馈 intake | 运行上下文控制，不产生命理判断 |
