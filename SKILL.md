...
- 复盘优先级计算共用 import-only `scripts/retrospective_priority.py`；它只是 runtime helper，不是新的 gate、命令入口或人工审批替代。
...
3. **Retrospective Intake**: 运行 `scripts/create_retrospective_intake.py --manifest <RUN_DIR>/case_manifest.json`，把复盘问题、建议 `target_artifacts`、候选命令、blocked 候选修复计划、blocked 修复优先级、ready 候选批量 dry-run/promote 顺序、后置审计命令和“最小人工审批建议”写入 `<RUN_DIR>/calibration/retrospective-intake.md` 与 `<RUN_DIR>/runtime/retrospective_intake.json`；这些命令只辅助人工审批，不自动晋升。blocked 候选先按 `run_local_blocked_candidate_repair_plan` 和 `run_local_repair_priority_queue` 补真实去隐私证据，不能用模型推测补 `domain_evidence`。最小审批建议只回答先批准哪几条最能关闭当前可关闭门槛，不能替代人工判断，也不能把候选直接写入全局知识库。
...
- `<RUN_DIR>/data/<case>-flow-timing.json`: `flow_timing_report` 的机器可读每日流月流日事实；仅时间行动报告需要。
- `<RUN_DIR>/drafts/<case>-flow-timing.md`: 流月流日行动节奏源稿；仅时间行动报告需要。
- `<RUN_DIR>/delivery/<reader-title>-流月流日行动节奏.html`: `flow_timing_report` 主交付，面向每日查看；仅时间行动报告需要。
