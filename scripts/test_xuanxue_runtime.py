...
class RuntimeWorkflowTests(unittest.TestCase):
    ...
    def test_retrospective_intake_uses_knowledge_context_plan(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("blocked 候选修复计划", markdown)
            self.assertIn("这里只给人工修复路径，不自动补证据", markdown)
            self.assertIn("优先修复顺序", markdown)
            self.assertIn("修复后可推进", markdown)
            self.assertIn("REQ-RETRO-WRITING", markdown)
            self.assertIn("repair_action: 补齐 domain_evidence", markdown)
            self.assertIn("修复后重跑", markdown)
            ...
            self.assertIn("REQ-RETRO-WRITING", local_candidate["matched_unsatisfied_requirements_after_repair"])
            self.assertIn("REQ-RETRO-ANY", local_candidate["matched_unsatisfied_requirements_after_repair"])
            self.assertGreater(local_candidate["repair_priority_score"], 0)
            self.assertIn("未满足", local_candidate["repair_priority_reason"])
            ...
            repair_plan = intake["run_local_blocked_candidate_repair_plan"]
            self.assertEqual(len(repair_plan), 1)
            self.assertEqual(repair_plan[0]["id"], "CR-20260613-reader-tone")
            self.assertEqual(repair_plan[0]["candidate_path"], "<RUN_DIR>\\retrospectives\\CR-20260613-reader-tone.candidate.json")
            self.assertTrue(any("补齐 domain_evidence" in item for item in repair_plan[0]["repair_actions"]))
            self.assertIn("REQ-RETRO-WRITING", repair_plan[0]["matched_unsatisfied_requirements_after_repair"])
            self.assertGreater(repair_plan[0]["repair_priority_score"], 0)
            self.assertIn("未满足", repair_plan[0]["repair_priority_reason"])
            self.assertIn("create_retrospective_intake.py", repair_plan[0]["recheck_command"])
            self.assertIn("--dry-run", repair_plan[0]["promotion_dry_run_command_after_fix"])
            evidence_templates = repair_plan[0]["domain_evidence_templates"]
            self.assertEqual(evidence_templates[0]["domain"], "writing")
            self.assertIn("json_fragment_template", evidence_templates[0])
            self.assertIn("writing", evidence_templates[0]["json_fragment_template"])
            self.assertIn("evidence_anchor", evidence_templates[0]["json_fragment_template"]["writing"])
            priority_queue = intake["run_local_repair_priority_queue"]
            self.assertEqual(priority_queue[0]["id"], "CR-20260613-reader-tone")
            self.assertIn("REQ-RETRO-WRITING", priority_queue[0]["matched_unsatisfied_requirements_after_repair"])
